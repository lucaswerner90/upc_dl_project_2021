...
class ImageCaptioningModel(nn.Module):
	def __init__(self, embed_size:int, vocab_size:int, caption_max_length:int,device):
		super(ImageCaptioningModel, self).__init__()
		self.encoder = Encoder(device)
		self.attention = Attention(image_features_dim=512, decoder_hidden_state_dim=embed_size, attention_dim=256,device=device)
