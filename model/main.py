...
class ImageCaptioningModel(nn.Module):
    def __init__(self, image_features_dim: int, embed_size: int, vocab: Vocabulary, caption_max_length: int,
                 attention_dim):
        super(ImageCaptioningModel, self).__init__()
        self.vocab = vocab
        self.vocab_size = len(self.vocab.word_to_index)
        self.encoder = Encoder()
        self.attention = Attention(image_features_dim=image_features_dim, decoder_hidden_state_dim=embed_size,
                                   attention_dim=attention_dim)
        self.decoder = Decoder(image_features_dim=image_features_dim, vocab_size=self.vocab_size,
                               hidden_size=embed_size, embed_size=embed_size)
